...
def twitter_connection():
    """
    Create a connection with dev.twitter.com to fetch results through OAuth API calls
    """
...
def cleanTxt(text):
    """
    Remove punctuation, emojis, filler links, etc from the actual tweet
    """
...
def extract_mentions(text):
    """
    Extract the mentions of other Twitter accounts in tweet bodies
    """
...
def extract_hastag(text):
    """
    Extract the hashtags in tweet bodies
    """
    text = re.findall("(#[A-Za-z0–9\d\w]+)", text)
    return text
...
def getSubjectivity(text):
    """
    Generate subjectivity score for tweet body
    """
...
def getPolarity(text):
    """
    Generate polarity score for tweet body
    """
...
def preprocessing_data(word_query, number_of_tweets, function_option):
    """
    Main controller function, that controls the flow of processing the downloaded tweets
    """
    if function_option == "Search By #Tag and Words":
        posts = tweepy.Cursor(api.search_tweets, q=word_query, count=200, lang="en", tweet_mode="extended").items(
            (number_of_tweets))

    if function_option == "Search By Username":
        posts = tweepy.Cursor(api.user_timeline, screen_name=word_query, count=200, tweet_mode="extended").items(
            (number_of_tweets))
...
def download_data(data, label):
    """
    Download the tweets from the UI/Frontend
    """
    current_time = datetime.datetime.now(pytz.timezone('Asia/Kolkata'))
    current_time = "{}.{}-{}-{}".format(current_time.date(), current_time.hour, current_time.minute,
                                        current_time.second)
